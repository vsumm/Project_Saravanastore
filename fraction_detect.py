...
def run(
    ...
    for path, im, im0s, vid_cap, s in dataset:
            ...
            if len(det):
                ...
                for c in det[:, 5].unique():
                    ...
                    async def send_telegram_message():
                        ...
                        message = f"Hi! Human Detected: {current_datetime_string}"

                        # Send the message
                        await bot.send_message(chat_id=chat_id, text=message)

                    # Call the function if 's' is 'human' or 'humans'
                    s = names[int(c)]
                    if s == "human" or s == "humans":
        ...
        # make new text file and append all log into text file and then convert it into json
        # Object detection logger statement
        logger_statement = ""

        if len(det) > 0:  # Check if there are any detections
            ...
            logger_statement = (
                f"Camera 1: {len(det)} human, Date: {date}, Time: {time}\n"
            )

        # Write the logger statement to the text file
        with open("Result.txt", "a") as file:
            file.write(logger_statement)
        ...
        for line in lines:
            ...
            if line.startswith("Camera"):
                data = {}
                data["camera 1"] = f"{len(det)} human"
                data["date"] = line.split("Date:")[1].split(",")[0].strip()
